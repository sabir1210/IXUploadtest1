...
namespace BDU.RobotWeb.EventCapture
{
    /// <summary>
    ...
    public class EventCapturingWebDriver : EventFiringWebDriver
    {
      ...
      // public event EventHandler<WebElementCapturedMouseEventArgs> ElementMouseLeaveCaptured;
        ...
        public event EventHandler<WebElementCapturedMouseEventArgs> ElementSubmitCaptured;
        ...
        protected virtual void OnFormElementClickCaptured(WebElementCapturedMouseEventArgs e)
        {
            try
            {
                /*
                                try
                                {
                                    string elmentName = e.Element.TagName;
                                }
                                catch (StaleElementReferenceException ex) {
                                    if (Convert.ToDouble(e.Button) <= 0)
                                    {
                                        return;
                                    }                   
                                }
                */
                //  e.ClientX
                //   WrappedDriver.SwitchTo().(ele);
                //  if(element.Text)
                ElementSubmitCaptured?.Invoke(this, e);
                //bool submitQualified = true;
                //if (WrappedDriver.FindElements(By.ClassName("parsley-required")).Count >=1)
                //    submitQualified = true;

                //if (submitQualified && e.Element != null && (e.Element.TagName.ToLower() == "button" || e.Element.TagName.ToLower() == "input" || e.Element.TagName.ToLower() == "a"))
                //{

                //    if (e.Element != null && e.Element.GetAttribute("type") == "submit")
                //    {
                //         WrappedDriver.ExecuteJavaScript("window.stop();");                        
                //        WrappedDriver.ExecuteJavaScript("window.event.preventDefault();");
                       
                //    }
                //    //if (WrappedDriver.PageSource.Contains("404") || WrappedDriver.FindElements(By.XPath("//*[contains(@class,invalid’)]")).Count > 0)
                //    //{
                //    //    //TODO Log
                //    //}
                //    //else

                //    // wait.Until(d => ((IJavaScriptExecutor)d).ExecuteScript(field.pms_field_expression)
                //    if(submitQualified)
        ...
        {
            //WrappedDriver.ExecuteJavaScript("'confirm('Do you want to sent to Servr ? ');'");
            //IAlert alert = WrappedDriver.SwitchTo().Alert();
        ...
        private void captureEvents()
        {
            ...
            while (eventCaptureEnabled )
            {
                ...
                try
                {
                   ...
                   if (UTIL.GlobalApp.currentIntegratorXStatus == UTIL.Enums.ROBOT_UI_STATUS.READY || UTIL.GlobalApp.currentIntegratorXStatus == UTIL.Enums.ROBOT_UI_STATUS.DEFAULT)
                   {
                        //string current = WrappedDriver.CurrentWindowHandle;
                    
                        //ReadOnlyCollection<string> handles = WrappedDriver.WindowHandles;
                        //if(handles != null && handles.Count>1 )
                        //{
                        //    if(current != handles[1].ToString())
                        //    WrappedDriver.SwitchTo().Window(handles[1].ToString());                          
                        //}

                      //  WrappedDriver.SwitchTo().Window(current);                      

                        // We couldn't find it
                       // WrappedDriver.SwitchTo().Window(current);
                        if (!WrappedDriver.ExecuteJavaScript<string>("return window.location.href;").Equals(WrappedDriver.Url))
                        {
                            WrappedDriver.Url = WrappedDriver.ExecuteJavaScript<string>("return window.location.href;");
                        }

                        // ArrayList<String> wid = new ArrayList<String>(WrappedDriver.CurrentWindowHandle());
                       ...
                       // driver.switchTo().window(newTb.get(0));
                        ...
                        IWebElement aElement = WrappedDriver.ExecuteJavaScript<IWebElement>("var activeElement; try {activeElement = document.activeElement;}catch (e) {activeElement = document.body || null; } while (activeElement && activeElement.shadowRoot && activeElement.shadowRoot.activeElement) { activeElement = activeElement.shadowRoot.activeElement; }  return activeElement") as IWebElement;
                    if (aElement.TagName == "iframe")
                    {  
                            // WrappedDriver.SwitchTo(aElement);
                            WrappedDriver.SwitchTo().Frame(aElement);                           
                        }
                            ...
                            switch (type.ToLower())
                            {
                                case "click":
                                       // WrappedDriver.ExecuteJavaScript("window.stop();");
                                        ...
                                        break;
                                    case "submit":
                                        raiseMouseEvent(evn, OnFormElementClickCaptured);
                                        break;
                                    //case "submit":
                                    //    raiseKeyboardEvent(evn, ElementSubmitCaptured);
                                    //    break;
                                   //case "contextmenu":
                                   //    // WrappedDriver.ExecuteJavaScript("confirm('Do you want to sent to Servr ? ');") ;                                       
                                   //    // IAlert alert = WrappedDriver.SwitchTo().Alert();
